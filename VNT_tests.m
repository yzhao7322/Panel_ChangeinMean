...
function [vnt_max,khat]=vnt_stat(yt)
% main statistics
% yt - panel data of interest
% vnt_max - test statistics
% khat - the detected change point
[T,N]=size(yt);
...
for i=1:T
    Stu=sum(dmy(1:i,:),1);
    au=Stu.*Stu-(i*(T-i))/(tauT*(T-tauT))*Stm2;
    Vseries(i,1)=sum(au);
end

[Vn_max,khat]=max(abs(Vseries));

%% compute the normalizating sequence 'w_bar'
...
%% vnt_stat
vnt_max=1/(T*sqrt(w_bar))*abs(Vn_max);

end


%% critical values derived by using bootstrap method (1)
function [cv_bboot]=gfmbootstrap_cv(data,rep)
% data - data of interest
% rep - replication times
...
%% critical values derived by using bootstrap method (2)
